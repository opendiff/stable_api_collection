...
apiClient_t *apiClient_create() {
    ...
    apiClient->basePath = strdup("https://api.stability.ai");
    ...
    apiClient->apiKeys_api_key_header_Authorization = NULL;
...
, list_t *apiKeys_api_key_header_Authorization
) {
    apiClient_t *apiClient = malloc(sizeof(apiClient_t));
    if(basePath){
    ...
    apiClient->progress_data = NULL;
    apiClient->response_code = 0;
    if(apiKeys_api_key_header_Authorization!= NULL) {
        apiClient->apiKeys_api_key_header_Authorization = list_createList();
        listEntry_t *listEntry = NULL;
        list_ForEach(listEntry, apiKeys_api_key_header_Authorization) {
            keyValuePair_t *pair = listEntry->data;
            keyValuePair_t *pairDup = keyValuePair_create(strdup(pair->key), strdup(pair->value));
            list_addElement(apiClient->apiKeys_api_key_header_Authorization, pairDup);
        }
    }else{
        apiClient->apiKeys_api_key_header_Authorization = NULL;
    }
...
void apiClient_free(apiClient_t *apiClient) {
    ...
    apiClient->progress_func = NULL;
    apiClient->progress_data = NULL;
    if(apiClient->apiKeys_api_key_header_Authorization) {
        listEntry_t *listEntry = NULL;
        list_ForEach(listEntry, apiClient->apiKeys_api_key_header_Authorization) {
            keyValuePair_t *pair = listEntry->data;
            if(pair->key){
                free(pair->key);
            }
        ...
        list_freeList(apiClient->apiKeys_api_key_header_Authorization);
    }
    free(apiClient);
}
...
void apiClient_invoke(apiClient_t    *apiClient,
    ...
    if(handle) {
        ...
        if (apiClient->apiKeys_api_key_header_Authorization != NULL)
        {
        list_ForEach(listEntry, apiClient->apiKeys_api_key_header_Authorization) {
