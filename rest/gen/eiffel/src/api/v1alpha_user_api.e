...
feature -- API Access
	...
	v1alpha_user_account : detachable ACCOUNT_RESPONSE_BODY
		...
		do
			...
			elseif attached { ACCOUNT_RESPONSE_BODY } l_response.data ({ ACCOUNT_RESPONSE_BODY }) as l_data then
				Result := l_data
			else
	...
	v1alpha_user_balance (organization: STRING_32): detachable BALANCE_RESPONSE_BODY
			-- balance
		...
		do
			...
			if l_response.has_error then
				last_error := l_response.error
			elseif attached { BALANCE_RESPONSE_BODY } l_response.data ({ BALANCE_RESPONSE_BODY }) as l_data then
				Result := l_data
			else
				create last_error.make ("Unknown error: Status response [ " + l_response.status.out + "]")
			end
